...
// 相机参数
const double SCREEN_Z = 1.0;        // 视平面 z 坐标
...
public:
    ...
    HitResult intersect(Ray ray)
    {
        ...
        if (dot(N, d) > 0.0f) N = -N;   // 获取正确的法向量,否则就有点像光线穿过了三角形的样子

        // 如果视线和三角形平行,法向量与射线方向垂直等同于视线与三角形平行
...
// 返回距离最近 hit 的结果,  看从这个像素出发有没有hit到物体并返回最近的hitresult
...
vec3 pathTracing(vector<Shape*>& shapes, Ray ray, int depth)
{
    ...
    if (r < res.material.specularRate)  // 镜面反射
    {
        ...
        //反射方向到随即方向的插值,roughness越大就越接近随机方向
                ...
                if (res.isHit)
                {
                    ...
                    else
                    {
                        ...
                        if (r < res.material.specularRate)  // 镜面反射
                        {
                            ...
                            //反射方向到随即方向的插值,roughness越大就越接近随机方向
                            randomRay.direction = mix(ref, randomRay.direction, res.material.roughness);
                            color = pathTracing(shapes, randomRay, 0) * cosine;
                        }
                        else if (res.material.specularRate <= r && r <= res.material.refractRate)    // 折射
                        {
                            //根据折射率算出折射方向
                        ...
                        {
                            vec3 srcColor = res.material.color;
                            vec3 ptColor = pathTracing(shapes, randomRay,0) * cosine;
                            color = ptColor * srcColor;    // 和原颜色混合
                        }
                        color *= BRIGHTNESS;
                    }
