...
struct BVHNode {
    ...
    BVHNode* right = NULL;
    int n, index;               // 叶子节点信息     n不为0代表叶子节点，n是储存的三角形个数，只有叶子节点储存三角形其他节点储存包围盒 index表示第几个三角形          
    vec3 AA, BB;                // 碰撞盒
};
...
BVHNode* buildBVHwithSAH(std::vector<Triangle>& triangles, int l, int r, int n) {
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        // leftMin[i]: [l, i] 中最小的 xyz 值    ，找到左包围盒的AABB
        ...
        // rightMin[i]: [i, r] 中最小的 xyz 值，，，，找到右包围盒的AABB
        ...
        for (int i = l; i <= r - 1; i++) {
            ...
            float leftCost = leftS * (i - l + 1);//左包为的消耗就是做包围盒的表面积和做包围盒的三角形数量
            ...
            float rightCost = rightS * (r - i);//左包为的消耗就是做包围盒的表面积和做包围盒的三角形数量
...
HitResult hitBVH(Ray ray, std::vector<Triangle>& triangles, BVHNode* root) {
    ...
    // 和左右子树 AABB 求交,看是与树的左包围盒相交还是右包围盒相交,当然一条光线可能与左右盒子都相交了，因为物体时立体的
    float d1 = INF, d2 = INF;
    ...
    //BVH的建立是先序遍历建立的，但是求交不是，求交只在于找到最后的结果，然后一路返回就好了
